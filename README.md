...
*antar* (inner) + *agni* (fire). Inner fire, versioned.

A decision casebook built from primary sources. Every heuristic carries its reason and its
boundary conditions, or it doesn't go in.
...
| field | what it holds |
|---|---|
| **situation** | the moment you are in when you'd reach for this |
...
**No heuristic without its exceptions.**

Most collections of practical wisdom are heuristic dumps. They tell you what worked for someone,
somewhere, and leave you to discover the hard way that the domain didn't transfer.

Knowing *why* a rule works is the only thing that tells you *where it stops*. So the concept field
is mandatory, and the exceptions field is the actual product.
...
Syriac, Arabic and Latin, and the characters survived all four.
...
Narrowed to *utsarga* and *apavāda* — the general rule and the specific rule that overrides it —
plus the precedence orderings that resolve rule conflicts, Pāṇini stops being a grammarian and
...
`decisions/` keeps superseded calls on purpose. A reversal with its reasoning intact is evidence of
method. A quietly deleted one is marketing.
